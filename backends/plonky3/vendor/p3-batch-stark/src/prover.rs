...
use crate::common::{
    CommonData, ProverData, PublicLookup, get_perm_challenges, observe_public_lookups,
};
...
{
    prove_batch_with_public_lookups(config, instances, prover_data, &[])
}

#[instrument(skip_all)]
pub fn prove_batch_with_public_lookups<
    SC,
    #[cfg(debug_assertions)] A: for<'a> Air<DebugConstraintBuilderWithLookups<'a, Val<SC>, SC::Challenge>>
        + Air<SymbolicAirBuilder<Val<SC>, SC::Challenge>>
        + for<'a> Air<ProverConstraintFolderWithLookups<'a, SC>>
    ...
    SymbolicExpression<SC::Challenge>: From<SymbolicExpression<Val<SC>>>,
{
    ...
    #[cfg(debug_assertions)]
    {
        ...
        if public_lookups.is_empty() {
            ...
            // Public boundary lookups are verifier-evaluated and therefore have no AIR trace
            // rows for the generic multiset debugger to replay. Per-AIR lookup constraints are
            // still checked above; the verifier checks the combined global balance.
        }
    }
