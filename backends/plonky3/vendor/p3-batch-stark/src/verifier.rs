...
use p3_lookup::lookup_traits::{
    Kind, Lookup, LookupData, LookupError, LookupGadget, lookup_data_to_expr,
};
...
use crate::common::{CommonData, PublicLookup, get_perm_challenges, observe_public_lookups};
use crate::config::{
...
{
    verify_batch_with_public_lookups(config, airs, proof, public_values, common, &[])
}

#[instrument(skip_all)]
pub fn verify_batch_with_public_lookups<SC, A>(
    config: &SC,
    airs: &[A],
    proof: &BatchProof<SC>,
    public_values: &[Vec<Val<SC>>],
    common: &CommonData<SC>,
    public_lookups: &[PublicLookup<Val<SC>>],
) -> Result<(), VerificationError<PcsError<SC>>>
where
    SC: SGC,
    SymbolicExpression<SC::Challenge>: From<SymbolicExpression<Val<SC>>>,
    ...
    Challenge<SC>: BasedVectorSpace<Val<SC>>,
{
    ...
    observe_public_lookups::<SC>(&mut challenger, public_lookups);
    ...
    for public in public_lookups {
        let challenges_per_lookup = lookup_gadget.num_challenges();
        let challenge_pair = all_lookups
            .iter()
            .zip(&challenges_per_instance)
            .find_map(|(lookups, challenges)| {
                lookups.iter().enumerate().find_map(|(index, lookup)| {
                    matches!(&lookup.kind, Kind::Global(name) if name == &public.name).then(|| {
                        let start = index * challenges_per_lookup;
        ...
        let combined = public
            .values
            .iter()
            .fold(SC::Challenge::ZERO, |acc, value| {
                SC::Challenge::from(*value) + acc * beta
            });
        ...
        global_cumulative
            .entry(&public.name)
            .or_default()
            .push(SC::Challenge::from(public.multiplicity) * inverse);
    }
