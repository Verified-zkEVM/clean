...
/// A verifier-known contribution to a named global lookup bus.
///
/// The extended batch APIs bind this complete value to the Fiat-Shamir transcript.
#[derive(Clone, Debug)]
pub struct PublicLookup<F> {
    pub name: alloc::string::String,
    pub values: Vec<F>,
    pub multiplicity: F,
}

/// Bind public lookup contributions to the Fiat-Shamir transcript.
pub fn observe_public_lookups<SC: SGC>(
    challenger: &mut SC::Challenger,
    public_lookups: &[PublicLookup<Val<SC>>],
) where
    Challenge<SC>: BasedVectorSpace<Val<SC>>,
{
    if public_lookups.is_empty() {
        return;
    }
    challenger.observe_base_as_algebra_element::<Challenge<SC>>(Val::<SC>::from_usize(
        public_lookups.len(),
    ));
    for public in public_lookups {
        let name = public
            ...
            .collect::<Vec<_>>();
        challenger
            .observe_base_as_algebra_element::<Challenge<SC>>(Val::<SC>::from_usize(name.len()));
        challenger.observe_slice(&name);
        challenger.observe_base_as_algebra_element::<Challenge<SC>>(Val::<SC>::from_usize(
            public.values.len(),
        ));
        challenger.observe_slice(&public.values);
        challenger.observe_slice(&[public.multiplicity]);
    }
}
