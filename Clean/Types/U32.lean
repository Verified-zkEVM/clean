...
omit p_large_enough in
@[circuit_norm]
lemma eval_of_literal (env : VerifierEnvironment (F p)) (a b c d : Var field (F p)) :
...
lemma getElem_eval_toLimbs {F} [Field F] {env : VerifierEnvironment F} {x : U32 (Expression F)} {i : ℕ} (hi : i < 4) :
...
lemma eval_fromLimbs {F} [Field F] {env : VerifierEnvironment F} {v : Vector (Expression F) 4} :
