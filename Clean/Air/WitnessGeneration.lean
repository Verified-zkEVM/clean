...
/-- Chained generation for a transition component (`windowRows = 2`): the seed program produces
the first row, and every further row comes from running the component's own witness generators on
the row before it. -/
structure TransitionMode (F : Type) where
  /-- Committed rows before padding: the seed plus `rows - 1` chained successors. -/
  rows : ℕ
  /-- The full first row. Like `PreallocatedMode.input`, an index-aware Witness IR
  program over the serialized prover input. -/
  seed : Witgen.RowProgram F
  seed_valid : seed.Valid .proverInput
...
inductive Mode (F : Type) where
  ...
  | transition (mode : TransitionMode F)

/-- A semantic row used to extend one component to a power-of-two trace height. For a transition
component, `input` is ignored: padding rows continue the chain, so only `minimumRows` applies. -/
...
structure Config (F : Type) (ProverInput : TypeMap) [ProvableType ProverInput] where
  ...
  fuel : ℕ
  /-- Runtime-only prover hints made available to every witness-generator run. Hints feed
  only witness construction and never appear in the proof (`ProverHint`). -/
  hint : ProverHint F := ProverHint.empty F
...
private def completeRow (prepared : PreparedComponent F) (input : Array F)
    (data : ProverData F) (hint : ProverHint F) (origin : Option (Origin F) := none) :
  ...
  let values := witgenWithData data hint prepared.witgenOps input
...
/-- One transition chain step: running the generators on the current row appends cells
`[width, 2 * width)`, which are the next committed row. -/
private def transitionStep (prepared : PreparedComponent F) (data : ProverData F)
    (hint : ProverHint F) (current : Array F) : Except String (Array F) := do
  let window := witgenWithData data hint prepared.witgenOps current
  unless window.size = 2 * prepared.width do
    throw s!"generated window has width {window.size}, expected {2 * prepared.width}"
  return window.extract prepared.width window.size

/-- The committed rows of a transition table: the seed plus `count` chained successors. Every
row is its own complete input (`input_eq_rowWidth`), so `input = values` throughout. -/
private def transitionRows (prepared : PreparedComponent F) (data : ProverData F)
    (hint : ProverHint F) (seed : Array F) (count : ℕ) :
    Except String (List (GeneratedRow F)) := do
  let mut rows : Array (GeneratedRow F) := #[{ input := seed, values := seed }]
  let mut current := seed
  for _ in [0:count] do
    let next ← transitionStep prepared data hint current
    rows := rows.push { input := next, values := next }
    current := next
  return rows.toList
...
private def initializeTableInputs (proverInput : Array F) :
  ...
  | prepared :: components, mode :: modes => do
      let rows ← match mode with
        ...
        | .transition mode => do
            let seed := (mode.seed.eval 0 proverInput).toArray
            unless seed.size = prepared.inputWidth do
              throw s!"transition seed for component '{prepared.component.circuit.name}' has \
                width {seed.size}, expected {prepared.inputWidth}"
            pure [{ input := seed, values := seed }]
...
private def completeInitialTables (data : ProverData F) (hint : ProverHint F) :
    List (PreparedComponent F) → List (Mode F) → List (GeneratedTable F) →
      Except String (List (GeneratedTable F))
  | [], [], [] => pure []
  | prepared :: components, mode :: modes, table :: tables => do
      let rows ← match mode with
        | .transition mode =>
            match table.rows with
            | [seed] => transitionRows prepared data hint seed.input (mode.rows - 1)
            | _ => throw s!"transition component '{prepared.component.circuit.name}' must be \
                initialized with exactly its seed row"
        | .demand _ | .preallocated _ =>
            table.rows.mapM fun row => completeRow prepared row.input data hint row.origin
      let rest ← completeInitialTables data hint components modes tables
      return { rows } :: rest
  | _, _, _ => .error "generated-table count does not match ensemble component count"
...
/-- The interactions of one transition window, evaluated on the same concatenated two-row
environment that `Table.windowEnv` builds on the assembled trace. -/
private def windowInteractions (prepared : PreparedComponent F)
    (current next : GeneratedRow F) (data : ProverData F) : List (Interaction F) :=
  let environment := Environment.fromArray (current.values ++ next.values) data
  prepared.interactions.map (·.eval environment)

/-- A table's interactions: one per row for a flat component, one per adjacent pair of rows
for a transition component. -/
private def generatedInteractions (prepared : PreparedComponent F)
    (table : GeneratedTable F) (data : ProverData F) : List (Interaction F) :=
  if prepared.component.windowRows = 2 then
    (table.rows.zip table.rows.tail).flatMap fun (current, next) =>
      windowInteractions prepared current next data
  else
    table.rows.flatMap (rowInteractions prepared · data)
...
private def tableInteractions (data : ProverData F) :
  ...
  | prepared :: components, table :: tables => do
      ...
      return generatedInteractions prepared table data ++ rest
...
private def Mode.handles (prepared : PreparedComponent F) (mode : Mode F)
  ...
  -- a transition trace is determined by its seed and length; its own interactions are
  -- demands *on other tables*, never the other way around
  | .transition _ => false
...
private def createDemandRow (prepared : PreparedComponent F) (mode : DemandMode F)
    (demand : Demand F) (multiplicity : ℕ) (data : ProverData F) (hint : ProverHint F) :
    Except String (GeneratedRow F) := do
  let origin : Origin F := {
  ...
  completeRow prepared (← mode.input.eval demand.message multiplicity) data hint (some origin)
...
private def handleDemand (prepared : PreparedComponent F) (mode : DemandMode F)
  ...
  | .byMessage =>
      ...
      | some index =>
          ...
          let updated ← createDemandRow prepared mode demand (origin.multiplicity + demand.count) data hint
          return {
...
private def handlePreallocated (prepared : PreparedComponent F)
    (handlers : List PreallocatedHandler) (demand : Demand F) (table : GeneratedTable F)
    (data : ProverData F) (hint : ProverHint F) : Except String (TableMutation F) := do
  ...
  let updated ← completeRow prepared input data hint
  return {
...
private def handle (prepared : List (PreparedComponent F)) (config : Config F ProverInput)
  ...
  let tableMutation ← match mode with
    | .demand mode => handleDemand component mode demand table data config.hint
    | .preallocated mode => handlePreallocated component mode.handlers demand table data config.hint
    -- unreachable: `Mode.handles` never routes a demand to a transition table
    | .transition _ => throw s!"transition component '{component.component.circuit.name}' \
        cannot handle channel demands"
  return {
...
private def padTables (data : ProverData F) (hint : ProverHint F) :
  ...
  | prepared :: components, padding :: paddings, table :: tables => do
      ...
      let rest ← padTables data hint components paddings tables
      if prepared.component.windowRows = 2 then
        -- A transition trace is padded by continuing the chain, so the padded suffix satisfies
        -- the transition constraints by construction and `Padding.input` is ignored. The added
        -- interactions are the new windows, starting with the one straddling the old last row.
        if count = 0 then
          return { tables := table :: rest.tables, added := rest.added }
        let some last := table.rows.getLast?
          | throw s!"transition component '{prepared.component.circuit.name}' cannot be \
              padded from an empty trace"
        let mut rows : Array (GeneratedRow F) := #[]
        let mut added : Array (Interaction F) := #[]
        let mut current := last
        for _ in [0:count] do
          let values ← transitionStep prepared data hint current.values
          let next : GeneratedRow F := { input := values, values }
          added := added ++ (windowInteractions prepared current next data).toArray
          rows := rows.push next
          current := next
        return {
          tables := { table with rows := table.rows ++ rows.toList } :: rest.tables
          added := added.toList ++ rest.added
        }
      else
        let rows ← (List.replicate count ()).mapM fun _ =>
          completeRow prepared padding.input data hint
        return {
...
private def validateModes :
  ...
  | prepared :: components, mode :: modes, padding :: paddings => do
      ...
      -- the generation mode must match the component's window shape
      match mode with
      | .transition _ =>
          unless prepared.component.windowRows = 2 do
            throw s!"transition generation for component '{prepared.component.circuit.name}' \
              requires a two-row window, found windowRows = {prepared.component.windowRows}"
      | .demand _ | .preallocated _ =>
          unless prepared.component.windowRows = 1 do
            throw s!"component '{prepared.component.circuit.name}' spans \
              {prepared.component.windowRows} trace rows and requires transition generation"
      ...
      | some _, .transition _ =>
          throw s!"fixed-column component '{prepared.component.circuit.name}' cannot use transition generation"
      ...
      | .transition transition =>
          unless transition.seed.width = prepared.inputWidth do
            throw s!"transition seed for component '{prepared.component.circuit.name}' has \
              width {transition.seed.width}, expected {prepared.inputWidth}"
          unless 0 < transition.rows do
            throw s!"transition component '{prepared.component.circuit.name}' must commit at \
              least its seed row"
...
private def padAndBalance (ensemble : Ensemble F PublicIO) (config : Config F ProverInput)
  ...
  | fuel + 1, tables => do
      let mutation ← padTables data config.hint prepared config.padding tables
...
/-- Assemble generated rows into committed traces. `GeneratedRow.values` is one trace row for
both supported window shapes; wider windows are refused, since their witness block spans several
rows and the single-row-lookahead chain cannot express that. -/
private def assembleTables :
  ...
  | component :: components, generated :: generatedTables => do
      if component.windowRows ≤ 2 then
        ...
        throw "witness generation supports at most two-row windows"
  | _, _ => .error "generated-table count does not match ensemble component count"
...
def generate (ensemble : Ensemble F PublicIO) (config : Config F ProverInput)
  ...
  | .ok initialInputs =>
    ...
    match completeInitialTables data config.hint prepared config.modes initialInputs with
    | .error error => .error error
...
mirroring `Boundary.Entry.Holds` clause by clause. `find?` is unambiguous because `unique_names`
makes witness table names unique. -/
