...
/-- Chained generation for a transition component (`windowRows = 2`).

The seed program produces the full first row, and every further row is generated by the
component's own witness generators: under `Component.input_eq_rowWidth` the input is the
entire current row and the witness block is exactly the next row, so one generator run per
window extends the trace by one committed row. -/
...
For a transition component, `input` is ignored: its padding rows are not free but are
generated by continuing the transition chain, so only `minimumRows` applies. -/
...
/-- One transition chain step: run the component's witness generators on the current row.

Under `Component.input_eq_rowWidth` the accumulator is exactly the two-row window: cells
`[0, width)` are the current row, and the generators append cells `[width, 2 * width)` -- the
next committed row -- with later witnesses reading earlier ones through the accumulator. -/
...
/-- The committed rows of a transition table: the seed plus `count` chained successors, each
produced from the previous row. Every row is its own complete input (`input_eq_rowWidth`), so
`input = values` throughout. -/
...
/-- The interactions of one transition window, evaluated on the concatenated two-row
environment -- the same environment `Table.windowEnv` builds on the assembled trace, so the
incremental imbalance bookkeeping agrees with the final `channelsBalanced` check. -/
...
/-- A table's interactions: one instance per row for a flat component, one per adjacent
pair of rows for a transition component (`n` rows carry `n - 1` windows, matching
`Table.envs_length`). -/
...
private def padTables (data : ProverData F) (hint : ProverHint F) :
  ...
  | prepared :: components, padding :: paddings, table :: tables => do
      ...
      if prepared.component.windowRows = 2 then
        -- A transition trace is padded by continuing the chain: each padding row is generated
        -- from the row before it, so the padded suffix satisfies the transition constraints by
        -- construction. `Padding.input` is ignored -- there is no free choice of padding row.
        -- The added interactions are the new windows, starting with the one that straddles the
        -- old last row.
        if count = 0 then
...
Generated rows are committed rows for both supported window shapes: a flat component's rows are
produced independently, a transition component's by chaining (`transitionRows`), and in either
case `GeneratedRow.values` is one trace row of width `component.width`.

Wider windows (`windowRows > 2`) are refused: their witness block spans several rows, which the
single-row-lookahead chain cannot express. The guard is on the component's own `windowRows`, so
there is no tag a caller could set inconsistently with the circuit's actual footprint:
`Component.window_size` ties `windowRows` to `circuit.size`. -/
