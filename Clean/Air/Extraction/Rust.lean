...
private def dataSchemaToRust (component : ComponentProgram F) : String :=
  s!"DataSchema \{ name: {quoted component.name}, columns: &[{commaSep (component.dataColumns.map toString)}] }"
...
def programToRust (name : String) (program : Program F) : Except String String := do
  ...
  let dataSchemas := commaSep (program.components.map dataSchemaToRust)
