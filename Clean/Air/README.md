...
`FlatEnsemble.lean` defines flat AIR ensembles. A raw ensemble is structural: it has components, declared channels, and an optional verifier circuit. Its statement is the raw proof-system relation: there exists a witness whose table constraints hold and whose channel interactions are balanced. The ensemble file also defines `Air.Flat.EnsembleWitness`, verifier-table plumbing, and the generic `Soundness`, `SpecConsistency`, and `AssumptionsConsistency` predicates parameterized by external assumptions and specs. It also defines `FormalEnsemble`, which bundles a raw ensemble with external `Assumptions`, a `Spec`, and a proof of soundness (completeness is TODO).

`OrderedChannels.lean` contains the staged channel-soundness construction for ordinary lookup-like channels. It defines partial balance, ordered channels, and the induction that derives channel guarantees for a ordered-channels list of tables from constraints plus balanced interactions. Finally, it defines `SoundEnsemble` and proves that adding tables and channels to such an ensemble preserves soundness,
under light-weight channel-ordering conditions.

`Vm.lean` contains the VM-channel construction. VM components both pull from and push to one distinguished state channel, so their soundness is not the same as ordinary ordered lookup-channel soundness. This file packages VM components and the verifier, proves the special VM channel soundness theorem, and provides the constructor for adding a VM layer to an already sound flat ensemble.
