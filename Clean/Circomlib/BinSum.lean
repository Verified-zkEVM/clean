...
def circuit (n ops : ℕ) [hn : NeZero n] (hnout : 2^(nbits ((2^n - 1) * ops)) < p) :
    FormalCircuit (F p) (BinSumInput n ops) (fields (nbits ((2^n - 1) * ops))) Unit where
  ...
  soundness := by
    intros offset env input_var input h_input_eval idx h_assumptions h_constraints_hold
  ...
  completeness := by
    ...
    simp only [circuit_norm, main, Num2Bits.arbitraryBitLengthCircuit]
    have h_inputs_binary' := h_inputs_binary ()
