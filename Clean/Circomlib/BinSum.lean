...
def circuit (n ops : ℕ) [hn : NeZero n] (hnout : 2^(nbits ((2^n - 1) * ops)) < p) :
  ...
  soundness := by
    intros offset env yielded input_var input h_input_eval h_assumptions h_constraints_hold
  ...
  completeness := by
    intros witness_offset env yielded inputs_var h_witness_extends inputs h_inputs_eval h_inputs_binary
    simp only [circuit_norm, main, Num2Bits.arbitraryBitLengthCircuit]
