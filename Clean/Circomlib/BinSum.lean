...
def main (n : ℕ) (bits : Vector (Expression (F p)) n) : Circuit (F p) (Expression (F p)) := do
  let (sum, _) ← Circuit.foldlRange n (0, 1) fun (sum, e2) i => do
...
def circuit (n ops : ℕ) [hn : NeZero n] (hops : 0 < ops) (hnout : 2^(nbits ((2^n - 1) * ops)) < p) :
  ...
  soundness := by
    ...
    simp only [InputLinearSum.circuit, Num2Bits.arbitraryBitLengthCircuit] at h_constraints_hold
    ...
    obtain ⟨h_input_sum, ⟨ h_sum_bound, h_output_decomp⟩⟩ := h_constraints_hold
