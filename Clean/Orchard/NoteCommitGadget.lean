...
private theorem copyCheck13_zlast_zero_val_lt (env : Environment Ecc.Fp)
    (element : Var field Ecc.Fp) (offset : ℕ)
    (h : ConstraintsHold.Soundness env
      ((Utilities.LookupRangeCheck.CopyCheck.circuit 13 element).operations offset))
    (hzLast : (eval env ((Utilities.LookupRangeCheck.CopyCheck.circuit 13).output element offset))[13]
        = 0) :
    (show Ecc.Fp from eval env element).val < 2 ^ (K * 13) := by
  let zs : fields 14 Ecc.Fp :=
    eval env ((Utilities.LookupRangeCheck.CopyCheck.circuit 13).output element offset)
  let f : ℕ → Ecc.Fp := fun i => if hi : i < 14 then zs[i]'hi else 0
  have hspec := withHint_spec_of_soundness
    (Utilities.LookupRangeCheck.CopyCheck.circuit 13) env element offset trivial h
  simp only at hspec
  rcases hspec with ⟨hz0, hchain⟩
  have hsteps :
      ∀ i, i < 13 →
...
theorem yCanonicity_gate_spec_of_soundness (env : Environment Ecc.Fp)
    ...
    let canonOffset := copyOffset + (Utilities.LookupRangeCheck.CopyCheck.circuit 25 j).localLength copyOffset
    let bounds := (canonBitshift130 zs[0]).output canonOffset
    let row : Var NoteCommit.YCanonicity.Row Ecc.Fp :=
      { y := y, lsb := lsb, k0 := k0, k2 := k2, k3 := k3, j := zs[0],
        z1J := zs[1], z13J := zs[13], jPrime := bounds.1, z13JPrime := bounds.2 }
    NoteCommit.YCanonicity.Spec (eval env row) := by
  let k0 := (witnessShort y 1 9 (by norm_num [K])).output offset
  let k2Offset := offset + (witnessShort y 1 9 (by norm_num [K])).localLength offset
  let k2 := (witnessShort y 250 4 (by norm_num [K])).output k2Offset
  let k3Offset := k2Offset + (witnessShort y 250 4 (by norm_num [K])).localLength k2Offset
...
theorem yCanonicity_gate_facts_of_soundness (env : Environment Ecc.Fp)
    (y lsb : Var field Ecc.Fp) (offset : ℕ)
    (h : ConstraintsHold.Soundness env ((yCanonicity y lsb).operations offset)) :
    let k0 := (witnessShort y 1 9 (by norm_num [K])).output offset
    let k2Offset := offset + (witnessShort y 1 9 (by norm_num [K])).localLength offset
    ...
    let canonOffset := copyOffset + (Utilities.LookupRangeCheck.CopyCheck.circuit 25 j).localLength copyOffset
    let bounds := (canonBitshift130 zs[0]).output canonOffset
    NoteCommit.IsBool (eval env k3) ∧
      eval env zs[0] = eval env lsb + eval env k0 * 2 + eval env zs[1] * 1024 ∧
      eval env y = eval env zs[0] + eval env k2 * OfNat.ofNat (2 ^ 250) +
        eval env k3 * OfNat.ofNat (2 ^ 254) ∧
      eval env bounds.1 = eval env zs[0] + OfNat.ofNat (2 ^ 130) - NoteCommit.tP ∧
      (eval env k3 = 0 ∨ eval env k2 = 0) ∧
      (eval env k3 = 0 ∨ eval env zs[13] = 0) ∧
      (eval env k3 = 0 ∨ eval env bounds.2 = 0) := by
  let k0 := (witnessShort y 1 9 (by norm_num [K])).output offset
  let k2Offset := offset + (witnessShort y 1 9 (by norm_num [K])).localLength offset
  let k2 := (witnessShort y 250 4 (by norm_num [K])).output k2Offset
  let k3Offset := k2Offset + (witnessShort y 250 4 (by norm_num [K])).localLength k2Offset
  ...
  let canonOffset := copyOffset + (Utilities.LookupRangeCheck.CopyCheck.circuit 25 j).localLength copyOffset
  let bounds := (canonBitshift130 zs[0]).output canonOffset
  have hSpec := yCanonicity_gate_spec_of_soundness env y lsb offset h
...
theorem main_g_piece_value (G : Generators) (Q : SWPoint Pallas.curve)
    ...
    let commitInput : Var (Sinsemilla.CommitDomain.Input 8) Ecc.Fp :=
      { pieces := #v[cells.a, cells.b, cells.c, cells.d, cells.e, cells.f, cells.g, cells.h],
        r := input.rcm }
    let pieces : Vector Ecc.Fp 8 := (eval env commitInput).pieces
    let (_gdX, _gdYbit, _pkdX, _pkdYbit, _v, rho, psi) := noteScalars (eval env input)
    pieces[6] = ((rho / 2 ^ 254 % 2 + (psi % 2 ^ 249) * 2 : ℕ) : Ecc.Fp) := by
  let cells := (assignMessageCells input).output offset
  let commitOffset := offset + (assignMessageCells input).localLength offset
  let out := (commitWithZs G Q hQ R input cells).output commitOffset
  let z1g := (HVec.get _ out.zs ⟨6, by decide⟩)[1]
  ...
  have hp := main_commitInput_piece_values env input offset
  have hdecomp := main_message_piece_decomposition_facts G Q hQ R env input offset h
  have hg0 := main_g0_eq_rho_high_bit G Q hQ R env input offset h
  have hpsi := main_psi_subpiece_bits G Q hQ R env input offset h
  simp only at hp hdecomp hg0 hpsi ⊢
  rcases hp with ⟨_hA, _hB, _hC, _hD, _hE, _hF, hpG, _hH⟩
  rcases hdecomp with ⟨_hBdec, _hDdec, _hEdec, hGdec, _hh1, _hHdec⟩
  rcases hGdec with ⟨_hg0Bool, hGdec⟩
  rw [hpG, hGdec, hg0]
  simp only [noteScalars, circuit_norm] at hpsi ⊢
  rw [Nat.cast_add, Nat.cast_mul, ← hpsi.1]
  ring

theorem main_h_piece_value (G : Generators) (Q : SWPoint Pallas.curve)
    (hQ : Q ≠ 0) (R : MulFixed.FixedBase) (env : Environment Ecc.Fp)
    (input : Var Input Ecc.Fp) (offset : ℕ)
    (h : ConstraintsHold.Soundness env ((main G Q hQ R input).operations offset)) :
    let cells := (assignMessageCells input).output offset
    let commitInput : Var (Sinsemilla.CommitDomain.Input 8) Ecc.Fp :=
      { pieces := #v[cells.a, cells.b, cells.c, cells.d, cells.e, cells.f, cells.g, cells.h],
        r := input.rcm }
    let pieces : Vector Ecc.Fp 8 := (eval env commitInput).pieces
    let (_gdX, _gdYbit, _pkdX, _pkdYbit, _v, _rho, psi) := noteScalars (eval env input)
    pieces[7] = ((psi / 2 ^ 249 % 32 + (psi / 2 ^ 254 % 2) * 32 : ℕ) : Ecc.Fp) := by
  let cells := (assignMessageCells input).output offset
  let commitInput : Var (Sinsemilla.CommitDomain.Input 8) Ecc.Fp :=
    { pieces := #v[cells.a, cells.b, cells.c, cells.d, cells.e, cells.f, cells.g, cells.h],
      r := input.rcm }
  ...
  have hdecomp := main_message_piece_decomposition_facts G Q hQ R env input offset h
  have hpsi := main_psi_subpiece_bits G Q hQ R env input offset h
  simp only at hp hdecomp hpsi ⊢
  rcases hp with ⟨_hA, _hB, _hC, _hD, _hE, _hF, _hG, hpH⟩
  rcases hdecomp with ⟨_hBdec, _hDdec, _hEdec, _hGdec, _hh1, hHdec⟩
  rw [hpH, hHdec, hpsi.2]
  simp only [noteScalars, circuit_norm] at hpsi ⊢
