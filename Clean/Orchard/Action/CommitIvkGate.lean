...
def circuit : FormalAssertion Fp Input where
  ...
  soundness := by
    ...
    have hloA_val : (input_a + input_b0 * ((2 ^ 250 : ℕ) : Fp)).val
        = input_a.val + input_b0.val * 2 ^ 250 :=
      val_limb2 250 (by omega)
    have hloA_lt : (input_a + input_b0 * ((2 ^ 250 : ℕ) : Fp)).val < 2 ^ 254 := by
      rw [hloA_val]; omega
    have hcanonA : input_b1 = 1 →
    ...
    have hrecA : input_ak = (input_a + input_b0 * ((2 ^ 250 : ℕ) : Fp))
        + input_b1 * ((2 ^ 254 : ℕ) : Fp) := by linear_combination -hak
    ...
    -- ===== nk side (3-limb low part `b2 + c·2^5 + d0·2^245`, top bit `d1`) =====
    have hloC_inner_val : (input_b2 + input_c * ((2 ^ 5 : ℕ) : Fp)).val
        = input_b2.val + input_c.val * 2 ^ 5 :=
      val_limb2 5 (by omega)
    have hloN_val : ((input_b2 + input_c * ((2 ^ 5 : ℕ) : Fp))
          + input_d0 * ((2 ^ 245 : ℕ) : Fp)).val
        = input_b2.val + input_c.val * 2 ^ 5 + input_d0.val * 2 ^ 245 := by
      rw [val_limb2 245 (by rw [hloC_inner_val]; omega), hloC_inner_val]
    have hloN_lt : ((input_b2 + input_c * ((2 ^ 5 : ℕ) : Fp))
          + input_d0 * ((2 ^ 245 : ℕ) : Fp)).val < 2 ^ 254 := by
      rw [hloN_val]; omega
    have hcanonN : input_d1 = 1 →
        ((input_b2 + input_c * ((2 ^ 5 : ℕ) : Fp))
          + input_d0 * ((2 ^ 245 : ℕ) : Fp)).val < tPNat := by
      intro h1
      have hd0z : input_d0 = 0 := by
        rcases mul_eq_zero.mp hd1d0 with h | h
        · exact absurd (h1 ▸ h) one_ne_zero
        · exact h
      have hinner_lt : (input_b2 + input_c * ((2 ^ 5 : ℕ) : Fp)).val < tPNat := by
        have hb2cP_val : input_b2CPrime.val
            = (input_b2 + input_c * ((2 ^ 5 : ℕ) : Fp)).val + 2 ^ 140 - tPNat := by
    ...
    have hrecN : input_nk = ((input_b2 + input_c * ((2 ^ 5 : ℕ) : Fp))
          + input_d0 * ((2 ^ 245 : ℕ) : Fp)) + input_d1 * ((2 ^ 254 : ℕ) : Fp) := by
    ...
    have hb2_eq : input_b2.val = bitrange input_nk.val 0 5 := by
      have h1 : input_b2.val = bitrange ((input_b2 + input_c * ((2 ^ 5 : ℕ) : Fp))
          + input_d0 * ((2 ^ 245 : ℕ) : Fp)).val 0 5 := by
    ...
    have hc_eq : input_c.val = bitrange input_nk.val 5 240 := by
      have h1 : input_c.val = bitrange ((input_b2 + input_c * ((2 ^ 5 : ℕ) : Fp))
          + input_d0 * ((2 ^ 245 : ℕ) : Fp)).val 5 240 := by
    ...
    have hd0_eq : input_d0.val = bitrange input_nk.val 245 9 := by
      have h1 : input_d0.val = bitrange ((input_b2 + input_c * ((2 ^ 5 : ℕ) : Fp))
          + input_d0 * ((2 ^ 245 : ℕ) : Fp)).val 245 9 := by
        simp only [bitrange, hloN_val]; omega
