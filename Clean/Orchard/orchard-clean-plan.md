...
When soundness and completeness proofs get long, factor out these additional definitions:

- `main`, the `Circuit` itself.
- `Spec` and `Assumptions`, the contract.
- `elaborated`, the `ElaboratedCircuit`, a typeclass instance needed by soundness and
  `circuit`, defined by `:= by elaborate_circuit`.
...
In general, follow Halo2 file/chip organization and naming closely. For example, if
Halo2 knows ECC entry point methods `incomplete_add` and `add`, then Clean namespaces
should be `IncompleteAdd` and `Add`.
