...
The custom-gate `FormalAssertion`s live in `Clean.Orchard.NoteCommitGate` under
`Orchard.NoteCommit.Gate`, because those low-level gates are imported by
scalar-multiplication definitions while this entry circuit depends on `Sinsemilla.Domain`
(the `CommitDomain.WithZs` hash that exposes the running sums) which sits above
`ScalarMul`.
-/
...
structure Input (F : Type) where
  note : Orchard.NoteCommit.Input F
