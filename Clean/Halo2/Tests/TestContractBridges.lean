...
derive_contract_bridges addTest := Halo2.Ironwood.Ecc.Add.add

-- 1. the five expected bridge names exist with the right statements.
example : Halo2.Ironwood.Ecc.Add.add.Spec
    = fun input output _ => output.Valid ∧ output = input.p + input.q := addTest_spec_eq
example : Halo2.Ironwood.Ecc.Add.add.Assumptions
    = fun input => input.p.Valid ∧ input.q.Valid := addTest_assumptions_eq
example : Halo2.Ironwood.Ecc.Add.add.EnvAssumptions = fun _ _ => True := addTest_envAssumptions_eq
example : Halo2.Ironwood.Ecc.Add.add.ProverAssumptions = fun _ _ => True :=
  addTest_proverAssumptions_eq
example : Halo2.Ironwood.Ecc.Add.add.ProverSpec = fun _ _ _ => True := addTest_proverSpec_eq
...
example (input : Value Halo2.Ironwood.Ecc.Add.Inputs Halo2.Ironwood.Fp)
    (output : Value Orchard.Point Halo2.Ironwood.Fp) (w : Unit)
    (h : Halo2.Ironwood.Ecc.Add.add.Spec input output w) :
