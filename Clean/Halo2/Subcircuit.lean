...
/--
An operation-local law holds over a folded call stream exactly when it holds over
each round's call chunk.  Keeping the round input and region index in `foldState`
lets clients reason about one symbolic round instead of expanding a concrete fold.
-/
theorem FormalCircuit.foldOps_forall (property : Operation F → Prop) (m : ℕ) :
    (FormalCircuit.foldOps c toInput config init i₀ m).Forall property ↔
      ∀ j : Fin m,
        (((c j).call config
          (FormalCircuit.foldState c toInput config init i₀ j).1).operations
            (FormalCircuit.foldState c toInput config init i₀ j).2).Forall property := by
  induction m with
  | zero =>
      simp [FormalCircuit.foldOps]
  | succ m ih =>
      rw [FormalCircuit.foldOps, List.forall_append, ih]
      constructor
      · rintro ⟨hprefix, hlast⟩ j
        by_cases hj : (j : ℕ) = m
        · simpa only [hj] using hlast
        · exact hprefix ⟨j, by omega⟩
      · intro hall
        exact
          ⟨fun j => hall ⟨j, by omega⟩,
            hall ⟨m, by omega⟩⟩

/-- `foldOps_forall`, specialized to exact lookup-selector activation accounting. -/
theorem FormalCircuit.foldOps_lookupRelevantSelectorActivationsExact (m : ℕ) :
    Operations.LookupRelevantSelectorActivationsExact
      ...
      ∀ j : Fin m,
        Operations.LookupRelevantSelectorActivationsExact
          (((c j).call config
            (FormalCircuit.foldState c toInput config init i₀ j).1).operations
              (FormalCircuit.foldState c toInput config init i₀ j).2) := by
  exact FormalCircuit.foldOps_forall c toInput config init i₀
    Operation.LookupRelevantSelectorActivationsExact m

/-- `foldOps_forall`, specialized to the lookup no-simple-selector law. -/
theorem FormalCircuit.foldOps_lookupInputsNoSimpleSelectors (m : ℕ) :
        ...
        Operations.LookupInputsNoSimpleSelectors
          (((c j).call config
            (FormalCircuit.foldState c toInput config init i₀ j).1).operations
              (FormalCircuit.foldState c toInput config init i₀ j).2) := by
  exact FormalCircuit.foldOps_forall c toInput config init i₀
    Operation.LookupInputsNoSimpleSelectors m

/--
The lookup-selector law for a serial fold reduces to one symbolic round at its
closed-form state.
-/
theorem FormalCircuit.foldCall_lookupRelevantSelectorActivationsExact (m : ℕ) :
    Operations.LookupRelevantSelectorActivationsExact
        ((FormalCircuit.foldCall c toInput config init m).operations i₀) ↔
      ∀ j : Fin m,
        Operations.LookupRelevantSelectorActivationsExact
  ...
  rw [FormalCircuit.foldCall_operations,
    FormalCircuit.foldOps_lookupRelevantSelectorActivationsExact]

/-- The lookup no-simple-selector law for a serial fold, round by round. -/
theorem FormalCircuit.foldCall_lookupInputsNoSimpleSelectors (m : ℕ) :
    Operations.LookupInputsNoSimpleSelectors
        ((FormalCircuit.foldCall c toInput config init m).operations i₀) ↔
      ∀ j : Fin m,
        Operations.LookupInputsNoSimpleSelectors
