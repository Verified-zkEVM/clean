...
private def cellEnd (cell : Cell) : Nat :=
  cell.row + 1

/-- Number of rows mentioned by layout metadata. -/
def usedRows (l : Layout) : Nat :=
  let fixedRows := l.fixedAssignments.foldl (fun acc (cell, _) => max acc (cellEnd cell)) 0
  let copyRows := l.copyConstraints.foldl (fun acc (left, right) => max acc (max (cellEnd left) (cellEnd right))) 0
  max l.nextRow (max fixedRows copyRows)
...
def ConfiguredCircuit.selectorCompressionInput {Config : Type} (c : ConfiguredCircuit Config) : SelectorCompressionInput :=
  ...
  let rows := max 1 (max (activationRows layout.selectorActivations) layout.usedRows)
