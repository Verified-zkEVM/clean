  ...
  withMainContext do
    ...
    if isSoundness then
      ...
      | some ``Soundness =>
        ...
        let names := [`i₀, `env, `yields, `checked, `input_var, `input, `h_input, `h_assumptions, `h_holds]
      ...
      | some ``GeneralFormalCircuit.Soundness =>
        ...
        let names := [`i₀, `env, `yields, `checked, `input_var, `input, `h_input, `h_holds]
        for name in names do
          evalTactic (← `(tactic| intro $(mkIdent name):ident))
      | _ => pure ()
      return
      ...
      | some ``Completeness =>
        ...
        let names := [`i₀, `env, `yields, `input_var, `h_env, `input, `h_input, `h_assumptions]
        for name in names do
          evalTactic (← `(tactic| intro $(mkIdent name):ident))
      | some ``FormalAssertion.Completeness =>
        evalTactic (← `(tactic| unfold FormalAssertion.Completeness))
        let names := [`i₀, `env, `yields, `input_var, `h_env, `input, `h_input, `h_assumptions, `h_spec]
        for name in names do
          evalTactic (← `(tactic| intro $(mkIdent name):ident))
