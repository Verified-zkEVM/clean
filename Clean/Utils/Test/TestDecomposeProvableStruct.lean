...
theorem test_decompose_mixed_auto {F : Type} [FiniteField F] (a : TestInputs F) (b : NestedInputs F) :
...
theorem test_decompose_nested_hypothesis {F : Type} [FiniteField F] (input : NestedInputs F)
...
theorem test_no_decompose_without_projection {F : Type} [FiniteField F] (input : TestInputs F) :
    input = input := by
...
theorem test_no_projection_struct {F : Type} [FiniteField F] (input : TestInputs F)
    (h : input = ⟨5, 3, 1⟩) :
    input = ⟨5, 3, 1⟩ := by
...
theorem test_multiple_no_projection {F : Type} [FiniteField F]
...
theorem test_struct_comparison {F : Type} [FiniteField F]
...
-- Test with struct comparison
theorem test_equality_not_triggar {F : Type} [FiniteField F]
