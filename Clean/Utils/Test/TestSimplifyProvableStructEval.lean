...
lemma test_eval_eq_struct_literal {F : Type} [Field F] (env : Environment F)
    ...
    (h : eval env.tape (TestInputs.mk x_var y_var z_var) = TestInputs.mk 1 2 3) :
    Expression.eval env.tape x_var = 1 ∧ Expression.eval env.tape y_var = 2 ∧ Expression.eval env.tape z_var = 3 := by
...
theorem test_struct_literal_eq_eval {F : Type} [Field F] (env : Environment F)
    (x_var y_var z_var : Var field F)
    (h : TestInputs.mk 1 2 3 = eval env.tape (TestInputs.mk x_var y_var z_var)) :
    1 = Expression.eval env.tape x_var ∧ 2 = Expression.eval env.tape y_var ∧ 3 = Expression.eval env.tape z_var := by
  fail_if_no_progress simplify_provable_struct_eval
  -- Now h should be literal = literal
  rw [TestInputs.mk.injEq] at h
  exact h
...
theorem test_eval_eq_struct_variable {F : Type} [Field F] (env : Environment F) (input : TestInputs F)
    (x_var y_var z_var : Var field F)
    (h : eval env.tape (TestInputs.mk x_var y_var z_var) = input) :
    TestInputs.mk (Expression.eval env.tape x_var) (Expression.eval env.tape y_var) (Expression.eval env.tape z_var) = input := by
...
theorem test_eval_in_conjunction {F : Type} [Field F] (env : Environment F) (x : F)
    (x_var y_var z_var : Var field F)
    (h : eval env.tape (TestInputs.mk x_var y_var z_var) = TestInputs.mk 1 2 3 ∧ x = 7) :
    Expression.eval env.tape x_var = 1 ∧ Expression.eval env.tape y_var = 2 ∧ Expression.eval env.tape z_var = 3 ∧ x = 7 := by
  simplify_provable_struct_eval
  -- now h should be literal = literal
  rw [TestInputs.mk.injEq] at h
  constructor
...
theorem test_nested_conjunctions_with_eval {F : Type} [Field F] (env : Environment F) (x : F)
    ...
    (h : (eval env.tape (TestInputs.mk x_var y_var z_var) = TestInputs.mk 1 2 3 ∧ x = 7) ∧
         eval env.tape (SimpleStruct.mk a_var b_var) = SimpleStruct.mk 8 9) :
    Expression.eval env.tape x_var = 1 ∧ Expression.eval env.tape a_var = 8 := by
  simplify_provable_struct_eval
...
theorem test_multiple_eval_expressions {F : Type} [Field F] (env1 env2 : Environment F)
    ...
    (h1 : eval env1.tape (TestInputs.mk x1_var y1_var z1_var) = TestInputs.mk 1 2 3)
    (h2 : eval env2.tape (SimpleStruct.mk a2_var b2_var) = SimpleStruct.mk 4 5) :
    Expression.eval env1.tape x1_var = 1 ∧ Expression.eval env2.tape b2_var = 5 := by
  simplify_provable_struct_eval
  -- now h should be a conjunction of literal = literal
...
theorem test_complex_eval {F : Type} [Field F] (env : Environment F)
    ...
    (h : eval env.tape (TestInputs.mk (a_var + b_var) (c_var * d_var) e_var) = TestInputs.mk 5 6 7) :
    Expression.eval env.tape (a_var + b_var) = 5 ∧
    Expression.eval env.tape (c_var * d_var) = 6 ∧
    Expression.eval env.tape e_var = 7 := by
  simplify_provable_struct_eval
...
theorem test_conjunction_with_base_and_non_base {F : Type} [Field F] (env : Environment F) (x : F)
    ...
    (h : (eval env.tape (TestInputs.mk x_var y_var z_var) = TestInputs.mk 1 2 3 ∧ x = 7) ∧
         eval env.tape s1 = eval env.tape s2) :
    Expression.eval env.tape x_var = 1 := by
  simplify_provable_struct_eval
  -- eval env.tape s1 = eval env.tape s2 should be intact
  simp only [TestInputs.mk.injEq] at h
  -- Since eval env.tape s1 = eval env.tape s2 should be intact, this simplification should fail
