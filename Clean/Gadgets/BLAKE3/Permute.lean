...
def main (state : Var BLAKE3State (F p)) : Circuit (F p) ProverHint (Var BLAKE3State (F p)) := do
  return Vector.ofFn (fun i => state[msgPermutation[i]])

instance elaborated: ElaboratedCircuit (F p) ProverHint BLAKE3State BLAKE3State where
  main := main
  localLength _ := 0
...
def circuit : FormalCircuit (F p) ProverHint BLAKE3State BLAKE3State :=
