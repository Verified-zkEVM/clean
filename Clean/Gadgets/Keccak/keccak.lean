...
def xor_u64 (a b : UInt64) : UInt64 := a ^^^ b

def not_u64 (a : UInt64) : UInt64 := a ^^^ 0xffffffffffffffff

def and_u64 (a b : UInt64) : UInt64 := a &&& b

def rol_u64 (value : UInt64) (left : Nat) : UInt64:=
...
def theta_c (state : Vector UInt64 25) : Vector UInt64 5 :=
    #v[
...
def theta_d (c : Vector UInt64 5) : Vector UInt64 5 :=
  #v[
...
def theta_xor (state : Vector UInt64 25) (d : Vector UInt64 5) : Vector UInt64 25 :=
  #v[
...
def theta (state : Vector UInt64 25) : Vector UInt64 25 :=
  let c := theta_c state
  let d := theta_d c
  theta_xor state d

def rho_phi (state : Vector UInt64 25) : Vector UInt64 25 :=
  #v[
...
def chi (b : Vector UInt64 25) : Vector UInt64 25 :=
  #v[
...
def iota (state : Vector UInt64 25) (rc : UInt64) : Vector UInt64 25 :=
  state.set 0 (xor_u64 rc <| state.get 0)


def keccak_round (state : Vector UInt64 25) (rc : UInt64) : Vector UInt64 25 :=
  let theta_state := theta state
  let rho_phi_state := rho_phi theta_state
  let chi_state := chi rho_phi_state
  iota chi_state rc

def keccak_f (state : Vector UInt64 25): Vector UInt64 25 :=
  let rec keccak_f_aux (state : Vector UInt64 25) (i : ℕ) : Vector UInt64 25 :=
    match i with
    | 0 => state
    | i + 1 =>
...
def state' := state.map (λ x => x |> U64.value_nat |> UInt64.ofNat)

def rc : U64 ℕ := ⟨235, 226, 178, 113, 2, 17, 87, 249⟩
#eval theta state' |> rho_phi |> chi
#eval keccak_f state' |>.map (λ x => x |> UInt64.toNat |> U64.decompose_nat_nat)
