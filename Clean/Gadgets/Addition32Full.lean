...
theorem soundness : Soundness (F p) (Inputs p) (Outputs p) add32_full assumptions spec := by
  rintro i0 env ⟨ x, y, carry_in ⟩ ⟨ x_var, y_var, carry_in_var ⟩ h_inputs as h

  -- rcases x with ⟨ x0, x1, x2, x3 ⟩
  -- rcases y with ⟨ y0, y1, y2, y3 ⟩
  let ⟨ x0, x1, x2, x3 ⟩ := x
  let ⟨ y0, y1, y2, y3 ⟩ := y
  -- rcases x_var with ⟨ x0_var, x1_var, x2_var, x3_var ⟩
  -- rcases y_var with ⟨ y0_var, y1_var, y2_var, y3_var ⟩
