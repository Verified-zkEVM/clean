...
-/
def main (x : Var field F) : Circuit F ProverHint (Var field F) := do
  -- When x ≠ 0, we need x_inv such that x * x_inv = 1
  -- When x = 0, x_inv can be anything (we use 0)
  let xInv ← witness fun env _ =>
...
instance elaborated : ElaboratedCircuit F ProverHint field field where
  main
...
theorem soundness : Soundness F ProverHint elaborated Assumptions (Spec (F:=F)) := by
  circuit_proof_start
  split
  · rename_i h_input
...
theorem completeness : Completeness F ProverHint elaborated Assumptions := by
  circuit_proof_start
  aesop

def circuit : FormalCircuit F ProverHint field field := {
