...
def main {sentences : PropertySet (F p)} (_order : SentenceOrder sentences) (input : Var Inputs (F p)) : Circuit sentences (Var U32 (F p))  := do
  let ⟨x, y⟩ := input
...
theorem soundness_to_u32 {sentences : PropertySet (F p)} {x y z : U32 (F p)}
  ...
  (h_eq :
    ...
    z.x3.val = x.x3.val ^^^ y.x3.val) : Spec (sentences := sentences) ∅ { x, y } z := by
  simp only [Spec]
...
theorem completeness {sentences : PropertySet (F p)} {order : SentenceOrder sentences} : Completeness (F p) sentences (elaborated order) CompletenessAssumptions := by
  ...
  simp only [CompletenessAssumptions, Assumptions, circuit_norm, U32.Normalized] at as
