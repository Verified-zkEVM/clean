...
def main {sentences : PropertySet (F p)} (_order : SentenceOrder sentences) (input : Var Inputs (F p)) : Circuit sentences (Var U64 (F p))  := do
  let ⟨x, y⟩ := input
  ...
  lookup (sentences := sentences) ByteXorTable (x.x0, y.x0, z.x0)
  lookup (sentences := sentences) ByteXorTable (x.x1, y.x1, z.x1)
  lookup (sentences := sentences) ByteXorTable (x.x2, y.x2, z.x2)
  lookup (sentences := sentences) ByteXorTable (x.x3, y.x3, z.x3)
  lookup (sentences := sentences) ByteXorTable (x.x4, y.x4, z.x4)
  lookup (sentences := sentences) ByteXorTable (x.x5, y.x5, z.x5)
  lookup (sentences := sentences) ByteXorTable (x.x6, y.x6, z.x6)
  lookup (sentences := sentences) ByteXorTable (x.x7, y.x7, z.x7)
  return z
...
theorem soundness {sentences : PropertySet (F p)} {order : SentenceOrder sentences} : Soundness (F p) (elaborated order) order Assumptions Spec := by
  intro i0 env state_var checked input_var input h_input h_as h_holds
