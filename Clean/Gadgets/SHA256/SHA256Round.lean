...
def circuit : FormalCircuit (F p) Inputs SHA256State where
  ...
  computableWitnesses := by
    ...
    -- constant but tactic-defined localLength metadata, exposed by rfl: the offset
    -- discharges below work with bare circuit_norm too, but reducing the metadata
    -- through these equations instead saves ~85k heartbeats on this proof
    ...
    all_goals
      exact FormalCircuit.toSubcircuit_computableWitnesses_onlyAccessedBelow_of_offset_eq _
        (by simp only [e1, e2, e3, e4, e5]) fun h_agrees => by
