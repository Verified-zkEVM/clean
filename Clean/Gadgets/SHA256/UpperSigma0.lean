...
instance elaborated : ElaboratedCircuit (F p) (fields 32) (fields 32) where
  main := main
  localLength _ := 32
  localLength_eq _ _ := by simp [circuit_norm, main, upperSigma0, xor3raw, carrySave]
  subcircuitsConsistent _ _ := by simp +arith [circuit_norm, main, upperSigma0, xor3raw, carrySave]
  channelsLawful := by intro x n; simp [circuit_norm, main, upperSigma0, xor3raw, carrySave]
...
theorem soundness : Soundness (F p) elaborated Assumptions Spec := by
  circuit_proof_start [upperSigma0, xor3raw, carrySave]
  ...
  set z : fields 32 (F p) :=
    Vector.map (Expression.eval env)
      (Vector.ofFn fun i : Fin 32 =>
        (rotr32 2 input_var)[i.val] + (rotr32 13 input_var)[i.val] + (rotr32 22 input_var)[i.val]
  ...
  have hpar : ∀ i : Fin 32, IsBool (input[(i + 2).val] + input[(i + 13).val] +
      input[(i + 22).val] - 2 * env.get (i₀ + i.val)) := by
    intro i; rw [IsBool.iff_mul_sub_one]
    have h := h_par_bool i; rw [h_r2 i, h_r13 i, h_r22 i] at h; linear_combination h
  have h_carry : ∀ i : Fin 32, env.get (i₀ + i.val) =
      input[(i + 2).val] * input[(i + 13).val] +
        input[(i + 22).val] *
          (input[(i + 2).val] + input[(i + 13).val] - 2 * (input[(i + 2).val] * input[(i + 13).val])) :=
    fun i => carry_eq_maj (ha (i + 2)) (ha (i + 13)) (ha (i + 22)) (hq i) (hpar i)
  have h_z : ∀ i : Fin 32, z[i] =
    ...
    rw [h_r2 i, h_r13 i, h_r22 i, h_carry i]; ring
...
theorem completeness : Completeness (F p) elaborated Assumptions := by
  ...
  · have hq := h_env_q i
    ...
    rw [hq, hr2 i, hr13 i, hr22 i]
    have hbool := maj_is_bool (ha (i + 2)) (ha (i + 13)) (ha (i + 22))
    rw [IsBool.iff_mul_sub_one] at hbool; linear_combination hbool
  · have hq := h_env_q i
    simp only [Vector.getElem_ofFn] at hq
    rw [hq, hr2 i, hr13 i, hr22 i]
    have hbool := parity_is_bool (ha (i + 2)) (ha (i + 13)) (ha (i + 22))
    rw [IsBool.iff_mul_sub_one] at hbool; linear_combination hbool
