...
lemma lift_val1 {x y b : (F p)} (x_byte : x.val < 256) (y_byte : y.val < 256) (b_bool : Clean.IsBool b) :
    (x + y + b).val = (x.val + y.val + b.val) := by
  have b_lt_2 := Clean.IsBool.val_lt_two b_bool
  field_to_nat

lemma lift_val2 {x b : (F p)} (x_byte : x.val < 256) (b_bool : Clean.IsBool b) :
...
theorem add32_soundness {x0 x1 x2 x3 y0 y1 y2 y3 carry_in c0 c1 c2 c3 z0 z1 z2 z3 : F p}
    ...
    (carry_in_bool : Clean.IsBool carry_in) (c0_bool : Clean.IsBool c0)
    (c1_bool : Clean.IsBool c1) (c2_bool : Clean.IsBool c2) (c3_bool : Clean.IsBool c3)
