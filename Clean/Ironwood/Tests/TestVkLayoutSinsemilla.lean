...
import Clean.Ironwood.Fixtures.SinsemillaLayout
...
namespace Halo2.Ironwood.Fixtures.Test.LayoutSinsemilla
...
def layoutG : Generators where
  S m :=
    let p : Halo2.Ironwood.Point Fp := { x := (tblX[m]! : Fp), y := (tblY[m]! : Fp) }
...
def layoutQ : Halo2.Ironwood.Point Fp :=
...
theorem layoutQ_onCurve : layoutQ.OnCurve := by
  show layoutQ.y ^ 2 = layoutQ.x ^ 3 + Halo2.Ironwood.pallasB
  decide
...
end Halo2.Ironwood.Fixtures.Test.LayoutSinsemilla
