...
import Clean.Ironwood.Fixtures.NoteCommitLayout
import Clean.Ironwood.Fixtures.NoteCommitSelMap
import Clean.Ironwood.Fixtures.NoteCommitParams
...
namespace Halo2.Ironwood.Fixtures.Test.LayoutNoteCommit
...
def ncG : Generators where
  S m :=
    let p : Halo2.Ironwood.Point Fp := { x := (ncTblX[m]! : Fp), y := (ncTblY[m]! : Fp) }
    if p.y ^ 2 = p.x ^ 3 + Halo2.Ironwood.pallasB then p else sinsemillaS0
  S_onCurve {m} _ := by
...
def ncQ : Halo2.Ironwood.Point Fp :=
...
theorem ncQ_onCurve : ncQ.OnCurve := by
  show ncQ.y ^ 2 = ncQ.x ^ 3 + Halo2.Ironwood.pallasB
  decide
...
def ncPointUnknown : Halo2.Ironwood.Point (FExpr Fp) := { x := .const 0, y := .const 0 }
...
end Halo2.Ironwood.Fixtures.Test.LayoutNoteCommit
