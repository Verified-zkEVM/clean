...
def round (w iter : ℕ) : FormalRegionCircuit Fp Config Config RoundInputs RoundOutput where
  ...
  ProverAssumptions input zPrev _ :=
    input.acc.Valid ∧ input.base.Valid ∧
  ...
  completeness := by
    ...
    obtain ⟨hAccV, hBaseV, hzPrev⟩ := prover_assumptions
...
/-- The round's output metadata: the second `add.call`'s output point plus this round's
running-sum cell. -/
@[circuit_norm]
theorem round_output (w iter : ℕ) (cfg : Config) (o : ℕ) (input : Var RoundInputs Fp)
    (self : RegionIndex) :
    (round w iter).output cfg o input self
      = { acc := (Add.add.call cfg.addConfig (o + 1)
            { p := input.acc,
              q := (Add.add.call cfg.addConfig o
                { p := { x := input.base.x, y := AssignedCell.of self o cfg.addConfig.yP },
                  q := input.acc }).output self }).output self,
          z := AssignedCell.of self (o + 2) cfg.zComplete } := rfl
      ...
      fold_sound (base := { x := input_base_x, y := input_base_y })
        ...
        (fun i hpre => by
          ...
          · rw [RegionCircuit.foldAcc_succ]
            simp only [FormalRegionCircuit.output_call', circuit_norm] at h2 ⊢
            exact h2
          · rw [RegionCircuit.foldAcc_succ]
            simp only [FormalRegionCircuit.output_call', circuit_norm] at h3 ⊢
            exact h3)
      ...
      fold_complete (base := { x := input_base_x, y := input_base_y })
        ...
        (fun i hpre => by
          ...
          simp only [round_envAssumptions_eq, round_assumptions_eq,
            ...
            circuit_norm, input_eq] at hleaf hder
          have hAV' := hAV
          rw [← point_eval_toEnvironment] at hAV'
          obtain ⟨hSpec, hPSacc, hPSz⟩ := hder ⟨hAV', hBV⟩ ⟨hAV, hBV, hzP⟩
          refine ⟨hleaf ⟨⟨hAV', hBV⟩, hAV, hBV, hzP⟩, ?_, ?_⟩ <;> beta_reduce
          · rw [RegionCircuit.foldAcc_succ]
            simp only [FormalRegionCircuit.output_call', circuit_norm,
              ← point_eval_toEnvironment] at hPSacc ⊢
            exact hPSacc
          · rw [show (↑i + 1) * 2 = ↑i * 2 + 2 from by ring]
            exact hPSz)
