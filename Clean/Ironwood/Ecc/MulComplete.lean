...
/-- The round's output record, reduced once (the `MulIncompleteRound.round_output`
pattern): the `acc` is the second `add.call`'s output point — the shared spelling the
fold's closed-form accumulator (`foldAcc`) chains through — and the `z` is the concrete
running-sum cell the position-chained z-facts land on. -/
@[circuit_norm]
      ...
      fold_sound (base := { x := input_base_x, y := input_base_y })
        ...
        (fun i hpre => by
          ...
          · rw [RegionCircuit.foldAcc_succ]
            simp only [circuit_norm] at h2 ⊢
            exact h2
          · rw [RegionCircuit.foldAcc_succ]
            simp only [circuit_norm] at h3 ⊢
            exact h3)
    ...
    obtain ⟨hCall, haccN, hzN⟩ :=
      fold_complete (base := { x := input_base_x, y := input_base_y })
        ...
        (fun i hpre => by
          ...
          · rw [RegionCircuit.foldAcc_succ]
            simp only [circuit_norm, ← point_eval_toEnvironment] at hPSacc ⊢
            exact hPSacc
          · rw [show (↑i + 1) * 2 = ↑i * 2 + 2 from by ring]
            exact hPSz)
