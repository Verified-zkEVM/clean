...
instance explicit : ExplicitCircuits (Gadgets.Addition32Full.main (p:=pBabybear) (emptyOrder (F pBabybear))) := by
  infer_explicit_circuits

@[reducible] def circuit32 {sentences : PropertySet (F pBabybear)} (order : SentenceOrder sentences) input := Gadgets.Addition32Full.main (p:=pBabybear) order input

example : ExplicitCircuit.localLength (circuit32 (emptyOrder (F pBabybear)) default) 0 = 8 := by
  -- rfl -- also works
  dsimp only [explicit_circuit_norm, explicit, assertBool]

example : ExplicitCircuit.output (circuit32 (emptyOrder (F pBabybear)) default) 0
...
example : ((circuit32 (emptyOrder (F pBabybear)) default).operations 0).SubcircuitsConsistent 0 :=
  ExplicitCircuits.subcircuitsConsistent ..

example (x0 x1 x2 x3 y0 y1 y2 y3 carryIn : Var field (F pBabybear)) env (i0 : ℕ) :
  Circuit.ConstraintsHold.Soundness env (emptyYields (F pBabybear)) (emptyChecked (F pBabybear)) ((circuit32 (emptyOrder (F pBabybear)) ⟨ ⟨ x0, x1, x2, x3 ⟩, ⟨ y0, y1, y2, y3 ⟩, carryIn ⟩).operations i0)
  ↔
