...
example (x0 x1 x2 x3 y0 y1 y2 y3 carryIn : Var field (F pBabybear)) env (i0 : ℕ) (yielded : Set (NamedList (F pBabybear))) :
  Circuit.ConstraintsHold.Soundness env yielded ((circuit32 ⟨ ⟨ x0, x1, x2, x3 ⟩, ⟨ y0, y1, y2, y3 ⟩, carryIn ⟩).operations i0)
  ↔
