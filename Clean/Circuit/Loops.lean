...
instance [NeZero m] : Inhabited (Fin m) := ⟨0⟩
...
theorem foldlAcc_const_succ (h_const_out : ConstantOutput fun (t : β × α) => circuit t.1 t.2)
  (i : ℕ) (hi : i + 1 < m) :
...
theorem foldlAcc_const (h_const_out : ConstantOutput fun (t : β × α) => circuit t.1 t.2)
  (i : ℕ) (hi : i < m) :
