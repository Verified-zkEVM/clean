...
variable {prop : Condition sentences}

lemma ConstantLength.length_eq_default {circuit : α → Circuit sentences β} (_ : ConstantLength circuit) [Inhabited α] (a : α) (n : ℕ) :
   (circuit a).localLength n = (circuit default).localLength 0 := by
  simp only [ConstantLength.localLength_eq]

def ConstantLength.fromConstantLength' [Inhabited β] (body : β × Fin m → Circuit sentences β)
    (h : ∀ (acc : β) (i i' : Fin m) n,
...
lemma forAll_flatten_abstract (circuit : Fin m → Circuit sentences β) (constant : ConstantLength circuit) :
...
lemma forAll_flatten (xs : Vector α m) {circuit : α → Circuit sentences β} (constant : ConstantLength circuit) :
...
private lemma ofFn_flatten_cons {circuit : α → Circuit sentences β} (constant : ConstantLength circuit) (x : α) (xs : Vector α m) (n : ℕ) :
...
namespace ForM
variable {circuit : α → Circuit sentences Unit} (xs : Vector α m) (constant : ConstantLength circuit) (n : ℕ)
...
namespace MapM
variable {circuit : α → Circuit sentences β} {xs : Vector α m} [constant: ConstantLength circuit]
  {prop : Condition sentences}
...
lemma mapM_cons (xs : Vector α n) (body : α → Circuit sentences β) (x : α) :
...
theorem mapFinRangeM_forAll_iff {circuit : Fin m → Circuit sentences β} [constant : ConstantLength circuit] :
...
namespace FoldlM
@[reducible]
def prod (circuit : β → α → Circuit sentences β) : β × α → Circuit sentences β := fun t => circuit t.1 t.2

variable {env : Environment F} {prop : Condition sentences} {xs : Vector α m}
  {circuit : β → α → Circuit sentences β} {init : β} {constant : ConstantLength (prod circuit)}
...
def foldlAcc (n : ℕ) (xs : Vector α m) (circuit : β → α → Circuit sentences β) (init : β) (j : Fin m) : β :=
...
section
variable {env : Environment F} {prop : Condition sentences} {m : ℕ}
  {Acc : ℕ → Type}
  {circuit : β → Fin m → Circuit sentences β} {init : β} {constant : ConstantLength (prod circuit)}
...
def forEach {m : ℕ} (xs : Vector α m) [Inhabited α] (body : α → Circuit sentences Unit)
...
lemma forEach_localYields_of_empty {m : ℕ} (xs : Vector α m) [Inhabited α]
  ...
  | nil =>
    ...
    rfl
  | cons x xs ih =>
    simp only [List.forM_cons, circuit_norm]
    rw [h_empty x offset]
...
def mapFinRange (m : ℕ) [NeZero m] (body : Fin m → Circuit sentences β)
    (_constant : ConstantLength body := by infer_constant_length) : Circuit sentences (Vector β m) :=
  Vector.mapFinRangeM m body

def foldl {m : ℕ} [Inhabited β] [Inhabited α] (xs : Vector α m) (init : β) (body : β → α → Circuit sentences β)
...
lemma forEach.soundness :
  ConstraintsHold.Soundness env yields checked ((forEach xs body constant).operations n) ↔
    ∀ i : Fin m, ConstraintsHold.Soundness env yields checked (body xs[i.val] |>.operations (n + i*(body default).localLength)) := by
  simp only [forEach, ConstraintsHold.soundness_iff_forAll' (yields:=yields) (checked:=checked)]
  rw [ForM.forAll_iff, ConstantLength.localLength_eq]

/-- variant of `forEach.soundness`, for when the constraints don't depend on the input offset -/
lemma forEach.soundness' :
  ConstraintsHold.Soundness env yields checked (forEach xs body constant |>.operations n) →
    ∀ x ∈ xs, ∃ k : ℕ, ConstraintsHold.Soundness env yields checked (body x |>.operations k) := by
  simp only [forEach, ConstraintsHold.soundness_iff_forAll' (yields:=yields) (checked:=checked), ForM.forAll_iff]
  intro h x hx
...
lemma forEach.completeness :
  ConstraintsHold.Completeness env yields ((forEach xs body constant).operations n) ↔
    ∀ i : Fin m, ConstraintsHold.Completeness env yields (body xs[i.val] |>.operations (n + i*(body default).localLength)) := by
  simp only [forEach, ConstraintsHold.completeness_iff_forAll' (yields:=yields)]
  rw [ForM.forAll_iff, ConstantLength.localLength_eq]
...
section map
variable {env : Environment F} {m n : ℕ} [Inhabited α] {xs : Vector α m}
  {body : α → Circuit sentences β} {constant : ConstantLength body} {prop : Condition sentences}
  {yields : YieldContext sentences} {checked : CheckedYields sentences}
...
@[circuit_norm ↓]
lemma map.soundness :
  ConstraintsHold.Soundness env yields checked (map xs body constant |>.operations n) ↔
    ∀ i : Fin m, ConstraintsHold.Soundness env yields checked (body xs[i.val] |>.operations (n + i*(body default).localLength)) := by
  simp only [map, ConstraintsHold.soundness_iff_forAll' (yields:=yields) (checked:=checked)]
  rw [MapM.forAll_iff, ConstantLength.localLength_eq]

@[circuit_norm ↓]
lemma map.completeness :
  ConstraintsHold.Completeness env yields (map xs body constant |>.operations n) ↔
    ∀ i : Fin m, ConstraintsHold.Completeness env yields (body xs[i.val] |>.operations (n + i*(body default).localLength)) := by
  simp only [map, ConstraintsHold.completeness_iff_forAll' (yields:=yields)]
  rw [MapM.forAll_iff, ConstantLength.localLength_eq]
...
section mapFinRange
variable {env : Environment F} {m n : ℕ} [NeZero m] {body : Fin m → Circuit sentences β}
  {constant : ConstantLength body} {prop : Condition sentences}
  {yields : YieldContext sentences} {checked : CheckedYields sentences}
...
lemma mapFinRange.soundness :
  ConstraintsHold.Soundness env yields checked (mapFinRange m body constant |>.operations n) ↔
    ∀ i : Fin m, ConstraintsHold.Soundness env yields checked (body i |>.operations (n + i*(body 0).localLength)) := by
  simp only [mapFinRange, ConstraintsHold.soundness_iff_forAll' (yields:=yields) (checked:=checked)]
  rw [MapM.mapFinRangeM_forAll_iff, ConstantLength.localLength_eq]

@[circuit_norm ↓]
lemma mapFinRange.completeness :
  ConstraintsHold.Completeness env yields (mapFinRange m body constant |>.operations n) ↔
    ∀ i : Fin m, ConstraintsHold.Completeness env yields (body i |>.operations (n + i*(body 0).localLength)) := by
  simp only [mapFinRange, ConstraintsHold.completeness_iff_forAll' (yields:=yields)]
  rw [MapM.mapFinRangeM_forAll_iff, ConstantLength.localLength_eq]
...
variable {env : Environment F} {m n : ℕ} [Inhabited β] [Inhabited α] {xs : Vector α m}
  {body : β → α → Circuit sentences β} {init : β} {constant : ConstantLength fun (t : β × α) => body t.1 t.2}
...
lemma foldl.soundness [NeZero m] :
  ConstraintsHold.Soundness env yields checked (foldl xs init body const_out constant |>.operations n) ↔
    ConstraintsHold.Soundness env yields checked (body init (xs[0]'(NeZero.pos m)) |>.operations n) ∧
    ∀ (i : ℕ) (hi : i + 1 < m),
      let acc := (body default xs[i]).output (n + i*(body default default).localLength);
      ConstraintsHold.Soundness env yields checked (body acc xs[i + 1] |>.operations (n + (i + 1)*(body default default).localLength)) := by
  simp only [foldl, ConstraintsHold.soundness_iff_forAll' (yields:=yields) (checked:=checked)]
  rw [FoldlM.forAll_iff_const constant const_out]

@[circuit_norm ↓]
lemma foldl.completeness [NeZero m] :
  ConstraintsHold.Completeness env yields (foldl xs init body const_out constant |>.operations n) ↔
    ConstraintsHold.Completeness env yields (body init (xs[0]'(NeZero.pos m)) |>.operations n) ∧
    ∀ (i : ℕ) (hi : i + 1 < m),
      let acc := (body default xs[i]).output (n + i*(body default default).localLength);
      ConstraintsHold.Completeness env yields (body acc xs[i + 1] |>.operations (n + (i + 1)*(body default default).localLength)) := by
  simp only [foldl, ConstraintsHold.completeness_iff_forAll' (yields:=yields)]
  rw [FoldlM.forAll_iff_const constant const_out]
...
variable {env : Environment F} {m n : ℕ} [Inhabited β]
  {body : β → Fin m → Circuit sentences β} {init : β}
...
lemma foldlRange.soundness :
  ConstraintsHold.Soundness env yields checked (foldlRange m init body constant |>.operations n) ↔
    ∀ i : Fin m,
    ConstraintsHold.Soundness env yields checked (body (FoldlM.foldlAcc n (Vector.finRange m) body init i) i
    |>.operations (n + i * (body default i).localLength)) := by
  simp only [ConstraintsHold.soundness_iff_forAll' (yields:=yields) (checked:=checked), foldlRange.forAll]

@[circuit_norm ↓]
lemma foldlRange.completeness :
  ConstraintsHold.Completeness env yields (foldlRange m init body constant |>.operations n) ↔
    ∀ i : Fin m,
    ConstraintsHold.Completeness env yields (body (FoldlM.foldlAcc n (Vector.finRange m) body init i) i
    |>.operations (n + i * (body default i).localLength)) := by
  simp only [ConstraintsHold.completeness_iff_forAll' (yields:=yields), foldlRange.forAll]
