...
namespace Circuit
variable {prop : Condition F ProverHint}

lemma ConstantLength.length_eq_default {circuit : α → Circuit F ProverHint β} (_ : ConstantLength circuit) [Inhabited α] (a : α) (n : ℕ) :
   (circuit a).localLength n = (circuit default).localLength 0 := by
  simp only [ConstantLength.localLength_eq]

def ConstantLength.fromConstantLength' [Inhabited β] (body : β × Fin m → Circuit F ProverHint β)
    (h : ∀ (acc : β) (i i' : Fin m) n,
...
lemma forAll_flatten_abstract (circuit : Fin m → Circuit F ProverHint β) (constant : ConstantLength circuit) :
...
lemma forAll_flatten (xs : Vector α m) {circuit : α → Circuit F ProverHint β} (constant : ConstantLength circuit) :
...
private lemma ofFn_flatten_cons {circuit : α → Circuit F ProverHint β} (constant : ConstantLength circuit) (x : α) (xs : Vector α m) (n : ℕ) :
...
namespace ForM
variable {circuit : α → Circuit F ProverHint Unit} (xs : Vector α m) (constant : ConstantLength circuit) (n : ℕ)
...
theorem forAll_iff {prop : Condition F ProverHint} :
...
namespace MapM
variable {circuit : α → Circuit F ProverHint β} {xs : Vector α m} [constant: ConstantLength circuit]
  {prop : Condition F ProverHint}
...
lemma ext_map_toList (f g : Circuit F ProverHint (Vector α n)) :
...
lemma mapM_cons (xs : Vector α n) (body : α → Circuit F ProverHint β) (x : α) :
...
theorem mapFinRangeM_forAll_iff {circuit : Fin m → Circuit F ProverHint β} [constant : ConstantLength circuit] :
...
namespace FoldlM
@[reducible]
def prod (circuit : β → α → Circuit F ProverHint β) : β × α → Circuit F ProverHint β := fun t => circuit t.1 t.2

variable {env : Environment F} {prop : Condition F ProverHint} {xs : Vector α m}
  {circuit : β → α → Circuit F ProverHint β} {init : β} {constant : ConstantLength (prod circuit)}
...
def foldlAcc (n : ℕ) (xs : Vector α m) (circuit : β → α → Circuit F ProverHint β) (init : β) (j : Fin m) : β :=
...
section
variable {env : Environment F} {prop : Condition F ProverHint} {m : ℕ}
  {Acc : ℕ → Type}
  {circuit : β → Fin m → Circuit F ProverHint β} {init : β} {constant : ConstantLength (prod circuit)}
...
def forEach {m : ℕ} (xs : Vector α m) [Inhabited α] (body : α → Circuit F ProverHint Unit)
    (_constant : ConstantLength body := by infer_constant_length) : Circuit F ProverHint Unit :=
  xs.forM body

def map {m : ℕ} (xs : Vector α m) (body : α → Circuit F ProverHint β)
    (_constant : ConstantLength body := by infer_constant_length) : Circuit F ProverHint (Vector β m) :=
  xs.mapM body

def mapFinRange (m : ℕ) [NeZero m] (body : Fin m → Circuit F ProverHint β)
    (_constant : ConstantLength body := by infer_constant_length) : Circuit F ProverHint (Vector β m) :=
  Vector.mapFinRangeM m body

def foldl {m : ℕ} [Inhabited β] [Inhabited α] (xs : Vector α m) (init : β) (body : β → α → Circuit F ProverHint β)
  (_const_out : ConstantOutput (fun (s, a) => body s a) := by
    simp only [circuit_norm]
    intros
    rfl)
  (_constant : ConstantLength (fun (s, a) => body s a) := by infer_constant_length)
   : Circuit F ProverHint β :=
  xs.foldlM body init

def foldlRange (m : ℕ) [Inhabited β] (init : β) (body : β → Fin m → Circuit F ProverHint β)
  (_constant : ConstantLength (fun (s, a) => body s a) := by
    apply Circuit.ConstantLength.fromConstantLength'
    try simp only [circuit_norm]
    try intros
    try ac_rfl) : Circuit F ProverHint β :=
  (Vector.finRange m).foldlM body init

section forEach
variable {env : Environment F} {m n : ℕ} [Inhabited α] {xs : Vector α m}
  {body : α → Circuit F ProverHint Unit} {constant : ConstantLength body} {prop : Condition F ProverHint}
...
section map
variable {env : Environment F} {m n : ℕ} [Inhabited α] {xs : Vector α m}
  {body : α → Circuit F ProverHint β} {constant : ConstantLength body} {prop : Condition F ProverHint}
...
section mapFinRange
variable {env : Environment F} {m n : ℕ} [NeZero m] {body : Fin m → Circuit F ProverHint β}
  {constant : ConstantLength body} {prop : Condition F ProverHint}
...
variable {env : Environment F} {m n : ℕ} [Inhabited β] [Inhabited α] {xs : Vector α m}
  {body : β → α → Circuit F ProverHint β} {init : β} {constant : ConstantLength fun (t : β × α) => body t.1 t.2}
...
variable {env : Environment F} {m n : ℕ} [Inhabited β]
  {body : β → Fin m → Circuit F ProverHint β} {init : β} {constant : ConstantLength fun (t : β × Fin m) => body t.1 t.2}
