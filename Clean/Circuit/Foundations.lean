...
theorem FormalCircuit.original_soundness (circuit : FormalCircuit F β α) :
  ...
  exact circuit.soundness offset env yielded b_var b h_input h_assumptions h_holds'
...
theorem FormalCircuit.original_completeness (circuit : FormalCircuit F β α) :
    ...
    ConstraintsHold env yielded (circuit.main b_var |>.operations offset) := by

  intro offset env yielded b_var b h_input h_assumptions h_env
  ...
  exact circuit.completeness offset env yielded b_var h_env' b h_input h_assumptions
...
theorem FormalAssertion.original_completeness (circuit : FormalAssertion F β) :
    ...
    circuit.Spec b → ConstraintsHold env yielded (circuit.main b_var |>.operations offset) := by

  intro offset env yielded b_var b h_input h_assumptions h_env h_spec
  apply Circuit.can_replace_completeness (circuit.subcircuitsConsistent ..) h_env
  have h_env' := Environment.can_replace_usesLocalWitnessesCompleteness (circuit.subcircuitsConsistent ..) h_env
  exact circuit.completeness offset env yielded b_var h_env' b h_input h_assumptions h_spec
