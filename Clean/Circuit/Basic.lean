...
def ConstraintsHold.Soundness (eval : Environment F) : List (Operation F) → Prop
  ...
  | .yield _ :: ops => ConstraintsHold.Soundness eval ops -- yield doesn't give any information to verifier
  | .subcircuit s :: ops =>
...
def ConstraintsHold.Completeness (eval : Environment F) : List (Operation F) → Prop
  ...
  | .yield { property, entry } :: ops =>
    property.Pred (entry.map eval) ∧ ConstraintsHold.Completeness eval ops
  | .subcircuit s :: ops =>
