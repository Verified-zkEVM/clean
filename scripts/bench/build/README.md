...
Whole-build metrics are recorded in the raw JSONL for debugging, but the rendered report uses heartbeat totals as the build-level signal.
...
The heartbeat metric reruns Lean's frontend for each successfully built module using the module's Lake setup file, re-elaborates each command with Mathlib's heartbeat-counting helper before elaborating it for real, and records the sum of user-facing heartbeat counts.
