...
Pushes to `main` run the heartbeat benchmark and store the resulting baseline measurements under `/var/lib/clean-bench/cache/baselines/heartbeats-v2/<sha>.jsonl`. A PR benchmark reuses that file when the requested base SHA has already been measured; otherwise it measures the baseline and stores it for future runs. The PR head is always measured from the requested checkout and is never reused from another SHA or checkout. Workflow artifacts include the summary report and a full alphabetically sorted module table.
